...
```{r}
connections %>% ggplot() +
  geom_sf(aes(col=child_time)) +
  scale_color_gradientn(colors=met.brewer("Hokusai1"))+
  theme_minimal()
ggsave(paste0("figs/",disp_f,"nodes.pdf"),device="pdf")
```

Now, I want to connect pairs of nodes, and look at the midpoint of their locations and the location of their MRCA. 
...
pairs <- expand.grid(tree$tip.label,tree$tip.label) %>% # get all pairs of tips
  ...
  filter(st_is_valid(line)==TRUE,st_is_valid(midtomrca)==TRUE,st_is_valid(midpoint)==TRUE)
```
...
pairs %>% ggplot(aes(col=MRCA_time)) +
  ...
  scale_color_gradientn(colors=met.brewer("Hokusai1"))+
  theme_minimal()
ggsave(paste0("figs/",disp_f,"MRCAs.pdf"),device="pdf")
```


```{r}
pairs %>% ggplot() +
  geom_point(aes(x=MRCA_x-mean(MRCA_x),y=error,col=as.factor(MRCA)))
ggsave(paste0("figs/",disp_f,"error_x.pdf"),device="pdf")
```

Does the age of the MRCA affect our estimate? Or is it maybe cancelled out by the number of descendant nodes?
...
ggarrange(p1,p2,p3)
ggsave(paste0("figs/",disp_f,"error.pdf"),device="pdf")
```
