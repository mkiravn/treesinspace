...
ngens <- 1000 # number of generations
...
write_delim(x = pars,file =paste0(as.character(Sys.Date()), "-Chapter3.tsv"),delim="\t" )
...
for (row in c(1:dim(pars)[1])) {
  ...
  if (pars[row, "sampling"]=="present"){
    modern_samples <- sampling(model, times = ngens, list(pop, 50))}else{
      modern_samples <- sampling(model, times = ngens, list(pop, 40))
    }

  if (pars[row, "sampling"]=="ancient"){
      ancient_samples <- sampling(model, times = c(ngens-200), list(pop, 10))}else{
        ancient_samples <- data.frame()
      }

  if (pars[row, "sampling"]=="staggered"){
    ancient_samples_staggered <- sampling(model,
                      times = runif(n = 10, min = ngens-400, max = ngens), list(pop, 1))}else{
  ...
  try_again(
    10,
    {
      slim(
      model,
      sampling = rbind(modern_samples, ancient_samples, ancient_samples_staggered),
  ...
  locs <- s %>%
    ...
    rowwise() %>%
    filter(is.na(var)==F) %>%
    mutate(location=st_cast(location,"POINT"),
           centroid_line=st_cast(st_union(centroid_inferred,location),"LINESTRING"),
           centroid_error=st_length(centroid_line)) %>%
    mutate(recursive_line=st_cast(st_union(recursive_inferred,location),"LINESTRING"),
...
######### Plotting
all_locations <- all_locations %>% filter(recursive_error>0,centroid_error>0)
...
all_locations %>%
  ...
  facet_grid(rows=vars(disp_fun),cols=vars(sampling,migration),labeller=label_both)+
  labs(x="years in past") +
  geom_smooth() +
  theme_minimal() -> rec.time

rec.time %>% ggsave(
  filename = paste0("figs/", as.character(Sys.Date()), "-inferring_location_rec.pdf"),
  device = "pdf",
  height = 5,
  width = 10
)
...
all_locations %>% filter(time>0) %>%
  ...
  geom_point(size=0.7) +
  geom_smooth(method="lm",col="lightpink",se=F,size=0.8)+
  facet_grid(rows=vars(disp_fun),cols=vars(sampling,migration),labeller=label_both)+
...
all_locations  %>% filter(time>0,rep==1,disp_fun=="brownian") %>% filter(st_is_valid(recursive_line)==T) %>%
  ggplot() +
  #geom_sf(data=filter(all_connections,rep==1,migration=="migrated",simplified=="simplified"),
  #        aes(geometry=parent_location),col="grey") +
  geom_sf(aes(geometry=recursive_line),col="red",lty=1,alpha=0.2) +
  geom_sf(aes(geometry=centroid_line),col="blue",lty=1,alpha=0.2) +
  geom_sf(aes(geometry=recursive_inferred),col="red",shape=21,fill="white") +
  geom_sf(aes(geometry=centroid_inferred),col="blue",shape=21,fill="white") +
  geom_sf(aes(geometry=location,col=time),size=1) +
  facet_grid(rows=vars(disp_fun),cols=vars(sampling,migration),labeller=label_both)+
  scale_colour_gradientn(colours = met.brewer(name = "Hokusai1"))+
  theme_minimal() -> points.plot


ggarrange(methods.comp,methods.time,points.plot,labels="auto",ncol=1,common.legend = T) %>%
  ggsave(
  filename = paste0("figs/", as.character(Sys.Date()), "-inferring_location_methods.pdf"),
  device = "pdf",
  height = 10,
  width = 10
)
...
all_locations %>% filter(time>0,disp_fun=="brownian") %>%
  ...
  ggplot(aes(y=error,x=migration,col=method,fill=sampling)) +
  geom_boxplot(alpha=0.6) +
  #facet_grid(rows=vars(method)) +
  theme(axis.text.x = element_text(angle = 45))+
  stat_summary(aes(col=method,fill=sampling),fun.y=mean, geom="point", shape=21, size=2,
               position = position_dodge(width = 0.75)) +
  theme_minimal() +
  scale_colour_grey()-> boxplot
...
compare_means(c(recursive_error,
                centroid_error)~sampling,
              all_locs_,
              method = "t.test",
              group.by = "migration") %>% select(-method,-p.format,-p.adj) %>%as_tibble() %>%
  gt() %>%
  tab_header(title = "Contrasts") %>% gtsave(paste0("figs/",
...
compare_means(c(recursive_error,
              ...
              all_locs_,
              method = "t.test",
              group.by = "sampling") %>% select(-method,-p.format,-p.adj)%>%
  as_tibble() %>%
  gt() %>%
...
compare_means(error~method, all_locs_pivot, method = "t.test",group.by = c("sampling","migration"))%>%
...
ggarrange(boxplot,labels="auto",nrow=1,common.legend = T) %>% ggsave(
  filename = paste0("figs/", as.character(Sys.Date()), "-inferring_location_boxplots.pdf"),
  device = "pdf",
  height = 5,
  width = 10
)
